...
<p><u>In order to be eligible to graduate, all of the boxes below must be filled in</u><br /><br /></p>
<?php
	...
	if(isset($_SESSION['user_id'])){
		...
		if($prog == 1){
			...
			if($blw_b_res['below_b'] < 3){
				...
				$core_query = "SELECT count(*) as core FROM enrollment WHERE user_id=" . $user . " AND course_dept='CSCI' AND course_num IN (6212,6221,6461) AND grade NOT IN ('F','IP', 'W')";
